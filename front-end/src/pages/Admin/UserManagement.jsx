...
import { PlusOutlined, LockOutlined, EditOutlined, DeleteOutlined, UnlockOutlined, SearchOutlined } from '@ant-design/icons';
...
const UserManagement = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'avatarUrl',
      render: (url, record) => (
        <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6, #60a5fa)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', overflow: 'hidden' }}>
          {url ? <img src={`http://localhost:8080/api/v1/files/${url}`} alt="" style={{width: '100%', height:'100%'}} /> : record.username[0].toUpperCase()}
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      render: (role) => {
        ...
        let color = '#3b82f6';
        let bg = '#eff6ff';
        if (roleName === 'Admin' || roleName === 'ADMIN') { color = '#9333ea'; bg = '#f3e8ff'; }
        else if (roleName === 'Teacher' || roleName === 'TEACHER') { color = '#ca8a04'; bg = '#fef9c3'; }
        return (
          <span style={{ color, background: bg, padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 800 }}>
            {roleName}
          </span>
        );
      }
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', gap: '8px' }}>
          <Button type="text" icon={<EditOutlined style={{ color: '#3b82f6' }} />} title="Chỉnh sửa" />
          <Button type="text" icon={record.isActive ? <LockOutlined style={{ color: '#fbbf24' }} /> : <UnlockOutlined style={{ color: '#10b981'}} />} title={record.isActive ? 'Khóa' : 'Mở khóa'} />
  ...
  return (
    <>
      ...
      <Modal 
        title={<span style={{fontSize: '20px', fontWeight: 800}}>Thêm người dùng mới</span>} 
        open={isModalOpen} 
      ...
      >
        <Form layout="vertical" style={{ marginTop: '20px' }}>
          ...
          <Form.Item label="Phân quyền (Role)" name="role" rules={[{ required: true }]}>
          ...
          <Form.Item label="Mật khẩu khởi tạo" name="password" rules={[{ required: true }]}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '32px' }}>
            ...
            <button type="submit" className="admin-btn-primary">Tạo mới</button>
          </div>
        </Form>
      </Modal>
    </>
