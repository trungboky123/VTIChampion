...
export default function ExamList() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setExams((prev) =>
        prev.filter((e) => (e.examId || e.id) !== examToDelete.id),
      );
  ...
  const totalAttempts = Object.values(examStats).reduce(
    ...
    totalExamsCount > 0
      ? (
          Object.values(examStats).reduce(
            (acc, curr) => acc + Number(curr.avgScore),
            0,
          ) / totalExamsCount
        ).toFixed(1)
      : "0.0";
  ...
  const filteredAndSortedExams = [...exams]
    .filter((exam) => {
      ...
      if (scoreFilter === "poor")
        return stats.attempts > 0 && Number(stats.avgScore) < 5;
      if (scoreFilter === "average")
        return (
          stats.attempts > 0 &&
          Number(stats.avgScore) >= 5 &&
          Number(stats.avgScore) < 8
        );
      if (scoreFilter === "good")
        return stats.attempts > 0 && Number(stats.avgScore) >= 8;
    ...
    <div className="exam-list-container" style={{ padding: 0 }}>
      <div className="page-actions">
        <div>
          ...
          <p className="page-subtitle">
            Quản lý và theo dõi các đề thi trực tuyến
          </p>
        </div>
        <button
      ...
      <div className="stats-grid">
        <div className="stat-item-box">
          ...
          <div>
            <div className="stat-val">{totalExamsCount}</div>
        ...
        <div className="stat-item-box">
          ...
          <div>
            <div className="stat-val">{globalAvgScore}</div>
              ...
              return (
                <div key={id || index} className="table-row-custom">
                  ...
                  <div className="exam-type-cell center">
                    <span
                      className={`type-badge ${exam.type === "Practice" ? "practice" : "test"}`}
                    >
                      {exam.type === "Practice" ? "Luyện tập" : "Đề thi"}
                    </span>
                  </div>
                  <div className="exam-attempts-cell center">
                    {stats.attempts}
                  </div>
      ...
      >
        <div style={{ padding: "10px 0" }}>
          <p style={{ fontSize: "15px", fontWeight: 600 }}>
            Bạn đang yêu cầu xoá bài thi:{" "}
            <span style={{ color: "#2563eb", fontWeight: 800 }}>
              "{examToDelete?.title}"
            </span>
          </p>
          <p style={{ fontSize: "13px", color: "#64748b", marginTop: "8px" }}>
            Hệ thống sẽ xoá vĩnh viễn dữ liệu này bao gồm cả cấu trúc câu hỏi
            bên trong. Bạn có chắc chắn muốn tiếp tục?
          </p>
        </div>
      </Modal>
    </div>
